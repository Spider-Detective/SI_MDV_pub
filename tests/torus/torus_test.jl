...
input = readdlm("d3p_torus_256_64_8_3");
...
for ind1 in 1:1
    for ind2 in (ind1+1):domNum 
        ...
        if calSep
            ...
            n_L2 = size(points2,2)
            ncenter = 10
            centers1, dist, d = findcenters(n_L1, points1[1,:], points1[2,:], points1[3,:], ncenter, seed, msglvl); 
            centers2, dist, d = findcenters(n_L2, points2[1,:], points2[2,:], points2[3,:], ncenter, seed, msglvl); 
            sep = Inf
            for m in 1:ncenter
                for n in 1:ncenter
                    dist = evaluate(Euclidean(), points1[:,centers1[m]], points2[:,centers2[n]])
                    if dist < sep
                        sep = dist
                    end
                end
            ...
            push!(separation, sep)
            #println("Minimum distance (separation): " ,sep)
        end
        ...
        if distRatio>=1
        #if sep>=2.5
            #println("Distance ratio larger than 1.")
            push!(farPairs, i)
        end
        println()
        i = i + 1
...
if writeIn
  ...
  open("MD_data_full_1_new.txt", "w") do f
      write(f, "Total subdomain N = 64. Inner radius = 3, outer radius = 8. \n")
